...
const BarangKeluar = () => {
  ...
  // PERBAIKAN (#2): Auto-currency formatting untuk Harga Jual Aktual.
  // State tetap menyimpan angka murni (tanpa titik) agar mudah dihitung/dikirim ke API,
  // sedangkan tampilan input diformat dengan pemisah ribuan + prefix "Rp" (lihat JSX di bawah).
  const handleHargaJualChange = (e) => {
    const digitsOnly = e.target.value.replace(/\D/g, ""); // buang semua karakter selain angka
    setFormData((prev) => ({ ...prev, harga_jual_aktual: digitsOnly }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...formData,
      jumlah: jumlahValue,
      harga_jual_aktual: Number(formData.harga_jual_aktual) || 0,
      id_user: id_user,
  ...
  return (
    <MasterLayout>
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        ...
        <button className="btn btn-danger rounded-pill px-4 shadow-sm fw-bold" onClick={() => setShowModal(true)}>
                ...
                transaksiKeluar.map((row) => {
                  ...
                  return (
                    <tr key={row.id_transaksi || row.id}>
                      <td className="ps-4">
                        <span className="fw-bold d-block">{new Date(row.tanggal_transaksi).toLocaleDateString("id-ID")}</span>
                        <small className="text-muted">{new Date(row.tanggal_transaksi).toLocaleTimeString("id-ID")} WIB</small>
                      </td>
                      <td className="fw-semibold text-dark">{namaBarangTampil}</td>
                      <td>
                        <span className="badge badge-keluar rounded-pill px-3 py-2 fw-bold">- {row.jumlah}</span>
                      </td>
                      ...
                      <td className="pe-4 small text-muted">{row.keterangan}</td>
                    </tr>
                  );
                })
              ) : (
      ...
      <div className={`modal fade ${showModal ? "show" : ""}`} style={{ display: showModal ? "block" : "none", zIndex: 1045 }} tabIndex="-1">
          ...
          <form onSubmit={handleSubmit} className="modal-content border-0 shadow-lg">
            ...
            <div className="modal-body">
              ...
              <div className="mb-3">
                ...
                <select name="id_barang" className="form-select rounded-3 shadow-sm" value={formData.id_barang} onChange={handleBarangChange} required>
              ...
              <div className="row">
                <div className="col-md-6 mb-3">
                  ...
                  <input type="number" name="jumlah" className="form-control rounded-3 shadow-sm" min="1" placeholder="0" value={formData.jumlah} onChange={handleInputChange} required />
                </div>
                <div className="col-md-6 mb-3">
                  ...
                  <div className="input-group rounded-3 shadow-sm overflow-hidden">
                    ...
                    <input
                      ...
                      placeholder="0"
                      value={formData.harga_jual_aktual ? Number(formData.harga_jual_aktual).toLocaleString("id-ID") : ""}
              ...
              <div className="row">
                <div className="col-md-6 mb-3">
                  ...
                  <input type="text" name="penerima" className="form-control rounded-3 shadow-sm" placeholder="Siapa pengambil?" value={formData.penerima} onChange={handleInputChange} required />
                </div>
                <div className="col-md-6 mb-3">
                  <label className="form-label small fw-bold">Total Penjualan</label>
                  <input type="text" className="form-control rounded-3 shadow-sm bg-light" value={formatRupiah(totalHarga)} readOnly />
                </div>
              </div>

              <div className="mb-0">
                <label className="form-label small fw-bold">Tujuan / Keperluan / Penerima</label>
                <textarea name="keterangan" className="form-control rounded-3 shadow-sm" rows="3" placeholder="Contoh: Digunakan divisi Humas" value={formData.keterangan} onChange={handleInputChange} required></textarea>
              </div>
            </div>
            ...
            <div className="modal-footer border-0 pt-0">
              ...
              <button type="submit" className="btn btn-danger rounded-pill px-4 fw-bold flex-grow-1" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
                    <i className="fa fa-spinner fa-spin me-2"></i>Memproses...
                  </>
                ) : (
                  "Proses Keluar"
                )}
