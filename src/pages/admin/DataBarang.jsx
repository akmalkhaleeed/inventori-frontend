...
const DataBarang = () => {
  ...
  // State Warning Harga (real-time)
  const [hargaWarning, setHargaWarning] = useState("");
  ...
  // ==========================================
  // Setiap kali harga_beli atau harga_jual berubah, cek apakah
  // harga_jual < harga_beli, lalu tampilkan warning di bawah input.
  useEffect(() => {
    const beli = parseInt(formData.harga_beli);
    const jual = parseInt(formData.harga_jual);

    if (!isNaN(beli) && !isNaN(jual) && formData.harga_jual !== "" && jual < beli) {
      setHargaWarning("Harga jual lebih kecil dari harga beli!");
    } else {
      setHargaWarning("");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // PERBAIKAN (Validasi #1): Jumlah/stok barang tidak boleh kurang dari 1
    // (tidak boleh 0 maupun negatif). Blokir submit dan tampilkan peringatan jelas.
    const stokValue = parseInt(formData.stok);
    ...
    // PERBAIKAN (Validasi #2): Harga jual tidak boleh lebih kecil dari harga beli.
    // Blokir submit dan tampilkan peringatan jelas.
    const hargaBeliValue = parseInt(formData.harga_beli) || 0;
    const hargaJualValue = parseInt(formData.harga_jual) || 0;
    if (hargaJualValue < hargaBeliValue) {
      Swal.fire({
        icon: "warning",
  ...
  const filteredData = barangs.filter((item) => item.nama_barang.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <MasterLayout>
      <div className="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-3">
        ...
        <div className="d-flex gap-2 flex-wrap">
          <input type="text" className="search-box" placeholder="Cari barang..." onChange={(e) => setSearchTerm(e.target.value)} />
                ...
                filteredData.map((row, i) => {
                  ...
                  return (
                    <tr key={validId}>
                      ...
                      <td>
                        <span className="badge bg-light text-dark border">{row.lokasi_rak || "-"}</span>
                      </td>
                      <td>
        ...
        <div className="custom-modal-overlay">
          <div className="custom-modal">
            <div className="modal-header d-flex justify-content-between align-items-center">
              <h5 className="fw-bold mb-0">{isEdit ? "Edit Data Barang" : "Tambah Barang"}</h5>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                <div className="mb-3">
                  ...
                  <input type="text" name="nama_barang" className="form-control" required value={formData.nama_barang} onChange={handleInputChange} />
                </div>

                <div className="row mb-3">
                  <div className="col-md-6 mb-3 mb-md-0">
                    <label className="form-label fw-semibold">Kategori</label>
                    <select name="id_kategori" className="form-select" required value={formData.id_kategori} onChange={handleInputChange}>
                      <option value="">- Pilih Kategori -</option>
                      {kategoris.map((kat) => {
                        // Sama halnya dengan ini, cari ID kategori yang benar
                        const katId = kat.id || kat.id_kategori;
                        return (
                          <option key={katId} value={katId}>
                            {kat.nama_kategori}
                          </option>
                        );
                      })}
                  ...
                  <div className="col-md-6">
                    ...
                    <select name="id_supplier" className="form-select" required value={formData.id_supplier} onChange={handleInputChange}>
                ...
                <div className="row mb-3">
                  <div className="col-md-4 mb-3 mb-md-0">
                    ...
                    <input type="number" name="stok" className="form-control" min="1" step="1" required value={formData.stok} onChange={handleInputChange} />
                    <small className="text-muted">Minimal 1</small>
                  </div>
                  <div className="col-md-4 mb-3 mb-md-0">
                    <label className="form-label fw-semibold">Satuan</label>
                    <input type="text" name="satuan" className="form-control" placeholder="pcs/box" required value={formData.satuan} onChange={handleInputChange} />
                  </div>
                  <div className="col-md-4">
                    <label className="form-label fw-semibold">Lokasi Rak</label>
                    <input type="text" name="lokasi_rak" className="form-control" placeholder="A-01" value={formData.lokasi_rak} onChange={handleInputChange} />
                  </div>
                </div>
                ...
                <div className="row">
                  <div className="col-md-6 mb-3 mb-md-0">
                    ...
                    <div className="input-group">
                      ...
                      <input type="number" name="harga_beli" className="form-control" required value={formData.harga_beli} onChange={handleInputChange} />
                    </div>
                  </div>
                  <div className="col-md-6">
                    ...
                    <div className="input-group">
                      <span className="input-group-text">Rp</span>
                      <input type="number" name="harga_jual" className={`form-control ${hargaWarning ? "is-invalid" : ""}`} required value={formData.harga_jual} onChange={handleInputChange} />
                    </div>
                    {/* Warning real-time muncul di sini saat harga jual < harga beli */}
                    {hargaWarning && (
              ...
              <div className="modal-footer">
                <button type="button" className="btn btn-light px-4" onClick={closeModal} disabled={isSubmitting}>
                  Batal
                </button>
                <button type="submit" className="btn btn-primary-custom px-4" disabled={isSubmitting}>
                  {isSubmitting ? (
                    <>
