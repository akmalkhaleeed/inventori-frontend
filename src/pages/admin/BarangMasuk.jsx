...
const BarangMasuk = () => {
    ...
    const [formData, setFormData] = useState({ 
        id_barang: '', jumlah: '', harga_beli: '', keterangan: '' 
    });
                        ...
                        <tbody>
                            {transaksi.map((t) => (
                                <tr key={t.id_transaksi}>
                                    ...
                                    <td><span className="badge badge-masuk px-3 py-2 fw-bold">+ {t.jumlah}</span></td>
                                    ...
                                    <td><small className="text-secondary">{t.nama_lengkap || t.name || (t.user && t.user.name) || 'Sistem'}</small></td>
                ...
                <div className="custom-modal-overlay">
                    <div className="custom-modal shadow-lg">
                        <div className="modal-header border-0 pb-3">
                            <h5 className="fw-bold"><i className="fa fa-arrow-down-long text-success me-2"></i> Tambah Stok</h5>
                        ...
                        <form onSubmit={handleSubmit} className="modal-body pt-0">
                            <div className="alert alert-info border-0 small py-2 mb-3">
                                ...
                                <a href="/admin/databarang" className="fw-bold text-decoration-none ms-1">Daftarkan di Master Barang</a>
                            </div>

                            <select name="id_barang" className="form-select mb-3 rounded-3 shadow-sm" required onChange={handleInputChange}>
                                <option value="">-- Cari Barang --</option>
                                {barangs.map(b => <option key={b.id_barang} value={b.id_barang}>{b.nama_barang} (Stok: {b.stok} {b.satuan})</option>)}
                            </select>

                            <div className="row">
                                <div className="col-6"><input type="number" name="jumlah" className="form-control mb-3 rounded-3 shadow-sm" placeholder="Jumlah" required onChange={handleInputChange}/></div>
                                <div className="col-6"><input type="number" name="harga_beli" className="form-control mb-3 rounded-3 shadow-sm" placeholder="Harga Beli" required onChange={handleInputChange}/></div>
                            </div>
                            ...
                            <button type="submit" className="btn btn-success w-100 fw-bold rounded-pill">Simpan Stok Masuk</button>
                        </form>
                    </div>
                </div>
            )}
