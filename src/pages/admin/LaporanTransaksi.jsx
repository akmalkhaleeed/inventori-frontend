      ...
      <div className="lap-area">
            ...
            <form onSubmit={handleFilterSubmit} className="row g-3">
              ...
              <div className="col-md-3">
                ...
                <select className="form-select rounded-3" value={idBarang} onChange={(e) => setIdBarang(e.target.value)}>
                  ...
                  {barangList.map((b) => (
                    <option key={b.id || b.id_barang} value={b.id || b.id_barang}>
                      {b.nama_barang}
                    </option>
                  ))}
                </select>
                  ...
                  {loading ? (
                    ...
                    dataDenganNo.map((item) => (
                      <tr key={item.id || item.id_transaksi || item.no}>
                        ...
                        <td>
                          <span className={`lap-badge-${item.jenis_final === "masuk" ? "masuk" : "keluar"}`}>{item.jenis_final.toUpperCase()}</span>
                        </td>
        ...
        <div className="print-only paper-cetak">
          ...
          <div className="judul-cetak">
            ...
            <p>
              Periode : {formatTanggalPrint(tglMulai)} - {formatTanggalPrint(tglSelesai)}
            </p>
          </div>
            ...
            <tbody>
              {dataDenganNo.map((item) => (
                <tr key={item.id || item.id_transaksi || item.no}>
                  ...
                  <td>
                    {item.jumlah_final} {item.satuan || item.barang?.satuan || ""}
                  </td>
                  <td>{formatRupiah(item.harga_final)}</td>
