...
const BarangKeluarPetugas = () => {
  ...
  // State menyimpan angka murni; tampilan diformat "Rp" + pemisah ribuan di JSX.
  ...
  const handleSubmit = async (e) => {
    ...
    // Validasi Dasar
    if (!formData.id_barang || formData.harga_jual_aktual <= 0) {
      Swal.fire({
        icon: "warning",
        title: "Data Tidak Valid!",
        text: "Pastikan barang dan harga jual sudah benar.",
        confirmButtonColor: "#f59e0b",
      });
      return;
    }
          ...
          <div className="card-custom">
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-3">
                ...
                <select name="id_barang" className="form-select text-dark fw-semibold" value={formData.id_barang} onChange={handleSelectBarang} disabled={isLoadingBarang} required>
              ...
              <div className="mb-3">
                ...
                <input type="number" name="jumlah" className="form-control" placeholder="Masukkan jumlah..." min="1" value={formData.jumlah} onChange={handleInputChange} required />
              </div>

              {/* KETERANGAN */}
              <div className="mb-3">
                <label className="form-label">Keterangan</label>
                <textarea name="keterangan" className="form-control" rows="3" placeholder="Contoh: Dijual ke pelanggan..." value={formData.keterangan} onChange={handleInputChange}></textarea>
              </div>
              ...
              {/* BUTTON SUBMIT */}
              <button type="submit" className="btn btn-keluar w-100 d-flex align-items-center justify-content-center" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
                    ...
                    history.map((row) => {
                      ...
                      return (
                        <tr key={row.id_transaksi}>
                          ...
                          <td>
                            <div className="fw-bold text-dark">{row.barang?.nama_barang || row.nama_barang || "ID: " + row.id_barang}</div>
                            <small className="text-muted">{row.barang?.satuan || row.satuan}</small>
                          </td>
                          <td className="fw-semibold text-dark small">Rp {Number(row.harga_beli).toLocaleString("id-ID")}</td>
                          <td className="fw-semibold text-success small">Rp {Number(row.harga_jual_aktual).toLocaleString("id-ID")}</td>
                          <td className="small text-muted">{row.keterangan || "-"}</td>
